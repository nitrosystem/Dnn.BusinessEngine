...
export function BindHtml(expressionService) {
    return {
        restrict: "A",
        replace: true,
        link: (scope, element, attrs) => {
                if (!attrs.bindHtml) return;

            function parseValue() {
                var content = expressionService.parseExpression(attrs.bindHtml, scope, "", true) || "";
            ...
            const matches = attrs.bindHtml.match(/(\w+)(\.([^{},=\-\+]+))?$/gm);
            _.forEach(matches, function (m) {
                const match = /(\w+)(\.([^{},=\-\+"]+))?/gm.exec(m);
                const property = match[0];
                scope.$watch(
...
export function BindImage($filter, expressionService) {
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            function parseValue() {
                var value = expressionService.parseExpression(attrs.bindImage, scope, "", true) || "";
                var content = "";
