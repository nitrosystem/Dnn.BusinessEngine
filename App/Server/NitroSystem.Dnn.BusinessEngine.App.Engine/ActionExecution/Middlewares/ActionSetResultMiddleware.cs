...
namespace NitroSystem.Dnn.BusinessEngine.App.Engine.ActionExecution.Middlewares
{
    public class ActionSetResultMiddleware : IEngineMiddleware<ActionRequest, ActionResponse>
    {
        ...
        public async Task<ActionResponse> InvokeAsync(IEngineContext context, ActionRequest request, Func<Task<ActionResponse>> next)
        {
            ...
            if (context.TryGet<object>("ResultData", out var actionResult) && !string.IsNullOrWhiteSpace(action.ActionResultDsl))
            {
                WithServiceResult(moduleData, actionResult, () =>
                {
                    var tokenizer = new Tokenizer(action.ActionResultDsl);
