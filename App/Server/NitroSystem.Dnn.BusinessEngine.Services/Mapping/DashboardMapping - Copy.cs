...
namespace NitroSystem.Dnn.BusinessEngine.BusinessLogic.Mapping
{
    public static class DashboardMapping
    {
        ...
        public static IEnumerable<DashboardViewModel> GetDashboardsViewModel(Guid scenarioID)
        {
            var result = new List<DashboardViewModel>();

            var dashboards = DashboardRepository.Instance.GetDashboards(scenarioID);
            foreach (var objDashboardView in dashboards)
            {
                result.Add(GetDashboardViewModel(objDashboardView));
            }

            return result;
        }

        public static DashboardViewModel GetDashboardViewModel(Guid dashboardID)
        {
        ...
        public static DashboardViewModel GetDashboardViewModelByModuleID(Guid moduleID)
        {
            var objDashboardView = DashboardRepository.Instance.GetDashboardViewByModuleID(moduleID);
        ...
        public static IEnumerable<DashboardPageViewModel> GetDashboardPagesViewModel(Guid scenarioID)
        {
            var result = new List<DashboardPageViewModel>();

            var pages = DashboardPageRepository.Instance.GetScenarioPages(scenarioID);
            foreach (var objDashboardPageInfo in pages)
            {
                var page = new DashboardPageViewModel();
                objDashboardPageInfo.CopyProperties(page);
                result.Add(page);
            }
