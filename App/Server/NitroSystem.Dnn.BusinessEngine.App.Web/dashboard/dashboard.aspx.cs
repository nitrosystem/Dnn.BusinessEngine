...
namespace NitroSystem.Dnn.BusinessEngine.App.Web
{
    public partial class Dashboard : System.Web.UI.Page
    {
        ...
        private PortalSettings Portal
        {
            get
            {
                int portalID = 1;// DashboardView != null ? DashboardView.PortalID : PortalSettings.Current.PortalId;
        ...
        public int DnnTabID
        {
            get
            {
                return DashboardView != null ? ModuleController.Instance.GetModulesByDefinition(Portal.PortalId, "BusinessEngine.Dashboard").Cast<DotNetNuke.Entities.Modules.ModuleInfo>().First(m => m.ModuleID == DashboardView.DnnModuleID).TabID : -1;
            }
        }
        ...
        public int DnnModuleID
        {
            get
            {
                return DashboardView != null ? DashboardView.DnnModuleID : -1;
            }
        }
        ...
        {
            if (this.UserInfo.UserID == -1 || !(UserInfo.IsSuperUser || string.IsNullOrEmpty(this.DashboardView.AuthorizationViewDashboard) || UserInfo.Roles.Any(r => this.DashboardView.AuthorizationViewDashboard.Split(',').Contains(r))))
            ...
            if (this.DashboardView != null)
            {
                ...
                var skin = DashboardSkinManager.GetSkin(this.DashboardView.ModuleID, HttpContext.Current);
                if (skin != null)
                {
                    var template = skin.DashboardTemplates.FirstOrDefault(t => t.TemplateName == this.DashboardView.Template);
