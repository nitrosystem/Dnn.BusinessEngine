...
using NitroSystem.Dnn.BusinessEngine.Studio.Engine.BuildModule.Dto;
...
namespace NitroSystem.Dnn.BusinessEngine.Studio.Engine.BuildModule
{
    public class ResourceMachine : IResourceMachine
    {
        ...
        private ModuleDto _buildModuleDto;
        ...
        private async Task<Dictionary<string, string>> LoadMultipleFilesAsync(
           int count,
        ...
        private bool ParseCondition(string condition)
        {
            ...
            var param = Expression.Parameter(typeof(ModuleDto));
            var expression = expressionTree.BuildExpression(param);

            var lambda = Expression.Lambda<Func<ModuleDto, bool>>(expression, param).Compile();
