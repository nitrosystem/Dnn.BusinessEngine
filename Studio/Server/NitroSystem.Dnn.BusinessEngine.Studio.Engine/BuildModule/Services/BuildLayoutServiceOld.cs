...
{
    public class BuildLayoutServiceOld : IBuildLayoutService
    {
        ...
        public BuildLayoutServiceOld(IServiceLocator serviceLocator, ISseNotifier notifier, IModuleFieldService moduleFieldService)
        {
        ...
        private async Task<string> ParseFieldTemplate(ModuleFieldDto field)
        {
            ...
            try
            {
                ...
                if (field.GlobalSettings.IsDisabledLayout && !string.IsNullOrEmpty(field.HiddenConditions))
                    fieldTemplate = fieldTemplate.Replace("[TOKENS]", @"[[IF:HiddenConditions != null && HiddenConditions != """":b-if=""!([[HiddenConditions]])""]] [TOKENS]");
                ...
                _onProgress.Invoke(_module.ScenarioName, $"Render template {field.FieldType} of {_module.ModuleName} module", (_fieldIndex * _progressStep));
            }
            catch (Exception exx)
            {
                throw exx;
