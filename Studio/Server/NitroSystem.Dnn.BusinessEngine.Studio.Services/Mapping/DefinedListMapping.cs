...
{
    public static class DefinedListMapping
    {
        private static IEnumerable<DefinedListItemInfo> ListItems { get; set; }

        #region Defined List Mapping

        public static IEnumerable<DefinedListViewModel> GetListsViewModel()
        {
            var definedLists = DefinedListRepository.Instance.GetAllLists();

            return GetListsViewModel(definedLists);
        }

        public static IEnumerable<DefinedListViewModel> GetListsViewModel(Guid scenarioID)
        {
        ...
        public static IEnumerable<DefinedListViewModel> GetListsViewModel(IEnumerable<DefinedListInfo> definedLists)
        {
        ...
        public static DefinedListViewModel GetListViewModel(DefinedListInfo objDefinedListInfo)
        {
            var config = new MapperConfiguration(cfg =>
            {
                ...
                .ForMember(dest => dest.Field, map => { map.PreCondition(source => source.FieldID != null); map.MapFrom(source => GetFieldDTO(source.FieldID.Value)); });
            });
        ...
        private static DefinedListFieldDTO GetFieldDTO(Guid fieldID)
        {
            var field = ModuleFieldRepository.Instance.GetField(fieldID) ?? new ModuleFieldInfo();

            return new DefinedListFieldDTO()
            {
