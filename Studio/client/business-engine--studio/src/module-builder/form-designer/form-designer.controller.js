...
import moduleTemplateWidget from "../form-designer/module-template.html";
import fieldTemplateWidget from "../form-designer/field-options/field-template.html";
import fieldEditWidget from "../form-designer/field-options/field-edit.html";
import fieldSettingsWidget from "../form-designer/field-options/field-settings.html";
import fieldShowConditionsWidget from "../form-designer/field-options/field-show-conditions.html";
import fieldConditionalValuesWidget from "../form-designer/field-options/field-conditional-values.html";
import fieldDataSourceWidget from "../form-designer/field-options/field-data-source.html";

export class ModuleFormDesignerController {
    constructor(
        $scope,
        $rootScope,
        studioService,
        ...
        this.moduleTemplateWidget = moduleTemplateWidget;
        ...
        $scope.$watch('$.currentField', (newVal, oldVal) => {
            if (!this.ignoreWatchCurrentField && newVal != oldVal && newVal && oldVal) {
                const cleanedCurrentField = this.globalService.cleanAngularHashkeyObject(newVal);
                const cleanedBackupField = this.globalService.cleanAngularHashkeyObject(this.backupFields[newVal.Id]);

                this.fieldsUnsaved[newVal.Id] = this.getSignificantChanges(
                    cleanedCurrentField,
                    cleanedBackupField,
                    ignoredFields
                );
        ...
        this.eventService.register('keydown', (e) => {
            ...
            { l: 'module.ModuleBuilderType', r: 0, _: this },
        ]);
        ...
        // var unregister = webSocketService.register(ModuleFormDesignerController, (data) => {
        //     $scope.$apply(() => {
        //         this.awaitAction = {
    ...
    onPageLoad() {
        ...
        let parent = this.globalService.getParameterByName("parent");
        parent = parent || id;
        ...
        this.apiService.get("Module", "GetModuleBuilder", { moduleId: (id || null), dashboardModuleId: parent })
            .then((data) => {
                ...
                if (this.dashboard) {
                    this.dashboardPreview = `${GlobalSettings.siteRoot + GlobalSettings.modulePath}dashboard.aspx?d=${this.dashboard.UniqueName}`;
                }
    ...
    saveCurrentField(targetField, isNewField) {
        ...
        if (!isNewField && this.fieldsUnsaved[targetField.Id] && !this.fieldsUnsaved[targetField.Id].length) {
            $defer.resolve(field);
        }
        else {
    ...
    getLastActivities() {
        _.keys(this.fieldsUnsaved).map(key => {
            const field = this.getFieldById(key);
            this.saveCurrentField(field, false);
        });
    }
