...
namespace NitroSystem.Dnn.BusinessEngine.Data.Repositories
{
    public class ModuleFieldSettingRepository
    {
        ...
        public IEnumerable<ModuleFieldSettingInfo> GetScenarioFieldSettings(Guid scenarioID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<ModuleFieldSettingInfo>();
                return rep.Find("WHERE FieldID in (SELECT FieldID FROM dbo.BusinessEngine_ModuleFields WHERE ModuleID in (SELECT ModuleID FROM dbo.BusinessEngine_Modules WHERE ScenarioID =@0))", scenarioID);
            }
        }
