...
namespace NitroSystem.Dnn.BusinessEngine.Data.Repositories
{
    public class ActionRepository
    {
        ...
        public IEnumerable<string> GetActionTypes(Guid moduleID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                return ctx.ExecuteQuery<string>(System.Data.CommandType.Text, "Select Distinct ActionType From dbo.BusinessEngine_Actions Where IsNull(IsServerSide,0) = 0 and ModuleID = @0", moduleID);
            }
        }
        ...
        public IEnumerable<ActionInfo> GetScenarioActions(Guid scenarioID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<ActionInfo>();
