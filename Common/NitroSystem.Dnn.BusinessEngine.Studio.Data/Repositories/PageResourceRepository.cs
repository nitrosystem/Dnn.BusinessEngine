...
namespace NitroSystem.Dnn.BusinessEngine.Data.Repositories
{
    public class PageResourceRepository
    {
        ...
        public void DeletePageResources(string resourceIDs)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<PageResourceInfo>();
                rep.Delete("Where ResourceID in (Select [RowValue] From dbo.ConvertListToTable(',',@0))", resourceIDs);
            }
        ...
        public IEnumerable<PageResourceView> GetModuleResourcesView(Guid moduleID)
        {
            ...
            if (result == null)
            {
                ...
                {
                    var rep = ctx.GetRepository<PageResourceView>();
                    result = rep.Find("Where ModuleID = @0", moduleID).OrderBy(r => r.LoadOrder);
                }
        ...
        public IEnumerable<PageResourceInfo> GetModuleResources(Guid moduleID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<PageResourceInfo>();
                return  rep.Find("Where ModuleID = @0", moduleID).OrderBy(r => r.LoadOrder);
            }
        }

        public IEnumerable<PageResourceView> GetActiveModuleResources(Guid moduleID)
        {
            var result = DataCache.GetCache<IEnumerable<PageResourceView>>(CachePrefix + "Active_" + moduleID);
            if (result == null)
            {
                using (IDataContext ctx = DataContext.Instance())
                {
                    var rep = ctx.GetRepository<PageResourceView>();
                    result = rep.Find("Where ModuleID = @0", moduleID).Where(a => a.IsActive);
                }
        ...
        public IEnumerable<PageResourceInfo> GetScenarioResources(Guid scenarioID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<PageResourceInfo>();
                return rep.Find("WHERE ModuleID in (SELECT ModuleID FROM dbo.BusinessEngine_Modules WHERE ScenarioID =@0)", scenarioID);
            }
        }
    }
}