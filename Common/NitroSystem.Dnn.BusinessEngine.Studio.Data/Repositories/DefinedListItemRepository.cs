...
namespace NitroSystem.Dnn.BusinessEngine.Data.Repositories
{
    public class DefinedListItemRepository
    {
        ...
        public IEnumerable<DefinedListItemInfo> GetScenarioListItems(Guid scenarioID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<DefinedListItemInfo>();
                return rep.Find("WHERE ListID in (SELECT ListID FROM dbo.BusinessEngine_DefinedLists WHERE ScenarioID =@0)", scenarioID);
            }
        }
