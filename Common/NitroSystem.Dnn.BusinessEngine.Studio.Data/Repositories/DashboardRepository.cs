...
namespace NitroSystem.Dnn.BusinessEngine.Data.Repositories
{
    public class DashboardRepository
    {
        ...
        public DashboardInfo GetDashboardInfoByModuleID(Guid moduleID)
        {
            var result = DataCache.GetCache<DashboardInfo>(CachePrefix + "Info_" + moduleID);
            if (result == null)
            {
                using (IDataContext ctx = DataContext.Instance())
                {
                ...
                DataCache.SetCache(CachePrefix + "Info_" + moduleID, result);
            }

            return result;
        }

        public DashboardView GetDashboardViewByModuleID(Guid moduleID)
        {
        ...
        public int GetDashboardType(Guid dashboardID)
        {
            return (this.GetDashboard(dashboardID) ?? new DashboardInfo()).DashboardType;
        }
        ...
        public IEnumerable<DashboardView> GetDashboards(Guid scenarioID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<DashboardView>();
                return rep.Find("Where ScenarioID = @0", scenarioID);
            }
        }
    }
}