...
namespace NitroSystem.Dnn.BusinessEngine.Data.Repositories
{
    public class ModuleFieldRepository
    {
        ...
        public IEnumerable<ModuleFieldInfo> GetChildFields(Guid fieldID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<ModuleFieldInfo>();
                return rep.Find("WHERE ParentID = @0", fieldID);
            }
        }

        public IEnumerable<string> GetFieldTypes(Guid moduleID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                return ctx.ExecuteQuery<string>(System.Data.CommandType.Text, "Select Distinct FieldType From dbo.BusinessEngine_ModuleFields Where ModuleID = @0", moduleID);
            }
        }
        ...
        public string GetModuleStyles(Guid moduleID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                return ctx.ExecuteScalar<string>(System.Data.CommandType.Text, "Select Distinct LayoutCss From dbo.BusinessEngine_Modules Where LayoutCss is not null and ModuleID =@0", moduleID);
            }
        }
        ...
        public IEnumerable<ModuleFieldInfo> GetScenarioFields(Guid scenarioID)
        {
            using (IDataContext ctx = DataContext.Instance())
            {
                var rep = ctx.GetRepository<ModuleFieldInfo>();
