...
namespace NitroSystem.Dnn.BusinessEngine.Core.ModuleBuilder
{
    public class ModuleData : JObject, IModuleData
    {
        ...
        {
            var value = _pageParams != null && _pageParams.ContainsKey(paramName) ? _pageParams[paramName] : null;
            return value;
        }
        ...
        public void SetPageParam(string pageUrl)
        {
            if (!string.IsNullOrWhiteSpace(pageUrl))
                _pageParams = new Uri(pageUrl).DecodeQueryParameters();
            //_pageParams = !string.IsNullOrWhiteSpace(pageUrl) ? new Uri(pageUrl) : null;
        }
        ...
        private void UpdateDbModuleData()
        {
            ...
            spParams.Add("@ConnectionID", _connectionID);
        ...
        {
            var spParams = new Dictionary<string, object>();
            spParams.Add("@ConnectionID", connectionID != null ? connectionID : _connectionID);
...
//    {
//        var val = data;
//        if (data.GetType() == typeof(string)) val = _expressionService.ParseExpression(data.ToString(), this);
