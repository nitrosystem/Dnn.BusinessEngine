...
namespace NitroSystem.Dnn.BusinessEngine.Core.ModuleBuilder
{
    public class ExpressionService : IExpressionService
    {
        ...
        public string ParseExpression(string expression, IModuleData moduleData, List<object> ignoreItems, bool ignoreVariable = false, string expressionParsingType = "", int moreThanOneParse = 0)
        {
            ...
            if (!ignoreVariableMatch && expressionParsingType != "static-expression")
            {
                ...
                if (ignoreItems == null) ignoreItems = new List<object>();
                ...
                foreach (Match match in matches)
                {
                    ...
                    if (ignoreItems.Contains(propertyPath)) continue;
                    ...
                    if (moduleData.IsVariable(variableName) || (ignoreVariable && (moduleData as JObject).SelectToken(variableName) != null))
                    {
                        ...
                        if (value != null && value != "") ignoreItems.Add(value);
            ...
            {
                return ParseExpression(expression, moduleData, ignoreItems, ignoreVariable, expressionParsingType, moreThanOneParse - 1);
            }
        }
